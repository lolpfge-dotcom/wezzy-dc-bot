...
const slashCommands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription("Start a giveaway now (uses a random prize from the pool unless you specify one).")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
  ...
  new SlashCommandBuilder()
    ...
    .setDescription("End the current active giveaway early and pick a winner.")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .toJSON(),
  new SlashCommandBuilder()
    .setName("giveaway-reroll")
    .setDescription("Pick a new winner for the most recently ended giveaway.")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .toJSON(),
  new SlashCommandBuilder()
    .setName("prize-add")
    .setDescription("Add a prize to the pool. The code/details are private and DM'd automatically to the winner.")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .addStringOption((o) => o.setName("name").setDescription("Public prize name (e.g. 'Roblox Premium Account')").setRequired(true).setMaxLength(100))
    .addStringOption((o) => o.setName("code").setDescription("The actual code, key, or credentials to DM the winner").setRequired(true).setMaxLength(1500))
    .toJSON(),
  new SlashCommandBuilder()
    .setName("prize-list")
    .setDescription("List prizes in the pool (names only — codes stay private).")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .toJSON(),
  new SlashCommandBuilder()
    .setName("prize-remove")
    .setDescription("Remove a prize from the pool by its position number (see /prize-list).")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
  ...
  new SlashCommandBuilder()
    ...
    .setDescription("Show giveaway schedule, prize pool size, and active count.")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .toJSON(),
  new SlashCommandBuilder()
    .setName("giveaway-auto")
    .setDescription("Turn the automatic Mon/Wed/Fri giveaways on or off.")
    .setDefaultMemberPermissions(ADMIN_ONLY)
    .setDMPermission(false)
    .addStringOption((o) =>
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      switch (interaction.commandName) {
        ...
        case "giveaway-start":
          return handleGiveawayStartCommand(interaction);

        case "giveaway-end":
          return handleGiveawayEndCommand(interaction);

        case "giveaway-reroll":
          return handleGiveawayRerollCommand(interaction);

        case "prize-add":
          return handlePrizeAddCommand(interaction);

        case "prize-list":
          return handlePrizeListCommand(interaction);

        case "prize-remove":
          return handlePrizeRemoveCommand(interaction);

        case "giveaway-status":
          return handleGiveawayStatusCommand(interaction);

        case "giveaway-auto":
          return handleGiveawayAutoCommand(interaction);
      }
      return;
    }
  ...
  const embed = adminEmbed("🛠️ Admin Commands").setDescription(
    ...
    },
    {
      ...
      value:
        ...
        "`/giveaway-end` — end the current giveaway early.\n" +
        "`/giveaway-reroll` — pick a new winner for the last one.\n" +
        "`/giveaway-auto on|off` — toggle the Mon / Wed / Fri auto-schedule.",
      inline: false,
    }
  );
  return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
...
function buildGiveawayEmbed(g, { ended = false, winner = null } = {}) {
  ...
  const desc = ended
    ? `### 🎁 ${g.prize}\n\n` +
      (winner ? `**Winner:** <@${winner}>` : "_No valid entries — no winner this time._") +
      `\n\n${endsLine}`
    : `### 🎁 ${g.prize}\n\n` +
      `React with 🎉 to enter.\n` +
...
async function giveawayTick() {
  ...
  if (!["Mon", "Wed", "Fri"].includes(weekday)) return;
...
async function handleGiveawayStatusCommand(interaction) {
  ...
  lines.push(`**Schedule:** Mon / Wed / Fri at ${String(CONFIG.GIVEAWAY_HOUR).padStart(2, "0")}:00 ${CONFIG.GIVEAWAY_TIMEZONE}`);
...
async function handleGiveawayAutoCommand(interaction) {
  ...
  return interaction.reply({
    content: giveawayData.autoEnabled
      ? "🟢 Automatic giveaways **enabled**. They'll fire Mon / Wed / Fri at the configured hour."
