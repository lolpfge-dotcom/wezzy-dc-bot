...
// Path to monitored channels config file
const CONFIG_FILE = path.join(__dirname, "monitored_channels.json")
...
function loadMonitoredChannels() {
  if (fs.existsSync(CONFIG_FILE)) {
    try {
      ...
      console.log("[v0] Loaded monitored channels:", JSON.stringify(parsed))
      return parsed
    } catch (error) {
      console.error("[v0] Error loading monitored channels:", error)
      return { outputChannel: null, monitoredChannels: [] }
    }
  } else {
    console.log("[v0] Config file doesn't exist, creating new one")
    const defaultConfig = { outputChannel: null, monitoredChannels: [] }
    saveMonitoredChannels(defaultConfig)
...
function saveMonitoredChannels(config) {
  try {
    ...
    console.log("[v0] Saved monitored channels:", JSON.stringify(config))
  } catch (error) {
    console.error("[v0] Error saving monitored channels:", error)
  }
}
...
async function verifyOrder(orderId) {
  try {
    ...
    console.log("[v0] Verifying order:", orderId)
...
client.on("messageCreate", async (message) => {
  console.log("[v0] Message received from:", message.author.tag, "in guild:", message.guildId)

  // Don't process bot messages or messages from DMs
  if (message.author.bot || !message.guild) {
    console.log("[v0] Skipping: bot message or DM")
    return
  }
  ...
  console.log("[v0] Is monitored:", isMonitored, "Guild:", message.guildId, "Channel:", message.channelId)
  console.log("[v0] Monitored channels:", JSON.stringify(monitoredConfig.monitoredChannels))
  console.log("[v0] Output channel set:", monitoredConfig.outputChannel)
  ...
  if (isMonitored && monitoredConfig.outputChannel) {
    try {
      ...
      if (!outputChannel) {
        console.log("[v0] Output channel not found")
        return
      }
      ...
      console.log(`[v0] ✅ Forwarded message from ${message.guild.name}#${message.channel.name}`)
    } catch (error) {
      console.error("[v0] Error forwarding message:", error)
    }
  }
...
client.once("ready", async () => {
  try {
    ...
    console.log("[v0] Startup - Monitored channels:", JSON.stringify(monitoredConfig.monitoredChannels))
    console.log("[v0] Startup - Output channel:", monitoredConfig.outputChannel)
  } catch (error) {
