...
// Configuration
const CONFIG = {
  BUYER_ROLE_ID: process.env.BUYER_ROLE_ID, // The role ID to give verified buyers
  VERIFICATION_CHANNEL_ID: process.env.VERIFICATION_CHANNEL_ID, // Channel where the panel will be
...
async function verifyOrder(orderId) {
  try {
    ...
    console.log('[v0] API Key:', process.env.SELLAPP_API_KEY ? 'Present' : 'Missing');
    ...
    // Sellapp API v2 returns nested structure
    ...
    console.log('[v0] Order status:', orderStatus);
    console.log('[v0] Customer email:', customerEmail);
    
    // Check if order is completed/paid (uppercase in API v2)
  ...
  if (interaction.customId === "order_verification_modal") {
    ...
    if (result.success) {
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription(result.error || "Could not verify your order.")
        .addFields(
          { name: "Order ID", value: orderId, inline: true },
...
client.on("messageCreate", async (message) => {
  if (message.content === "!setup-panel" && message.member.permissions.has("Administrator")) {
    const embed = new EmbedBuilder()
      ...
      .setThumbnail("https://hebbkx1anhila5yf.public.blob.vercel-storage.com/bild-C6Idn9oTQj2ABd2uhumQFB0DyYLHud.png") // Your snake W logo
