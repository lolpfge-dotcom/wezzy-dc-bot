...
const CONFIG = {
  ...
  WELCOME_BANNER_URL: process.env.WELCOME_BANNER_URL || null,
  WELCOME_BANNER_FILE: process.env.WELCOME_BANNER_FILE || null,
};
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    // Giveaway alerts toggle
    if (interaction.customId === "subscribe_giveaway") {
      await interaction.deferReply({ flags: MessageFlags.Ephemeral });
      if (!CONFIG.GIVEAWAY_PING_ROLE_ID) {
        return interaction.editReply({
          content: "❌ Giveaway alerts aren't configured yet. Ping an admin.",
        });
      }
      const role = interaction.guild.roles.cache.get(CONFIG.GIVEAWAY_PING_ROLE_ID);
      if (!role) {
        return interaction.editReply({ content: "❌ Giveaway role not found. Ping an admin." });
      }

      const member = interaction.member;
      try {
        if (member.roles.cache.has(CONFIG.GIVEAWAY_PING_ROLE_ID)) {
          await member.roles.remove(role);
          return interaction.editReply({
            content:
              "🔕 **Giveaway alerts turned OFF.**\n" +
              "You won't be pinged on new giveaways anymore.\n\n" +
              "_Tap **Giveaway Alerts** again any time to turn them back on._",
          });
        }
        await member.roles.add(role);
        return interaction.editReply({
          content:
            "🎉 **Giveaway alerts turned ON.**\n" +
            "You'll get pinged whenever a new giveaway goes live.\n\n" +
            "_Tap **Giveaway Alerts** again any time to turn them off._",
        });
      } catch (err) {
        console.error("[GIVEAWAY] Role toggle failed:", err.message);
        return interaction.editReply({
          content:
            "❌ I couldn't change your role. My role may be **below** the giveaway role — ask an admin to move it up.",
        });
      }
    }
...
function buildVerificationPanel() {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        name: "Extras",
        value:
          "Tap **Restock Alerts** to be pinged when products drop back in stock.\n" +
          "Tap **Giveaway Alerts** to be pinged when a new giveaway goes live.",
        inline: false,
      }
    )
    .setFooter({
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("subscribe_giveaway").setLabel("Giveaway Alerts").setStyle(ButtonStyle.Secondary).setEmoji("🎉"),
  ...
  if (winner) {
    ...
    if (deliveryStatus === "dm-failed") {
      ...
      if (adminChannelId) {
        ...
        if (adminChannel) {
          const adminPing = CONFIG.ADMIN_USER_ID ? `<@${CONFIG.ADMIN_USER_ID}> ` : "";
          await adminChannel.send({
            content: `${adminPing}⚠️ Giveaway prize couldn't be DM'd to <@${winner}> (DMs closed). Please deliver **${g.prize}** manually.`,
            allowedMentions: CONFIG.ADMIN_USER_ID
              ? { users: [CONFIG.ADMIN_USER_ID] }
              : { users: [] },
          }).catch(() => {});
        }
      }
...
// ---------------------------------------------------------------------------
// Welcome image generator (premium card — blurred avatar bg, orange accents)
// ---------------------------------------------------------------------------
...
// Lightweight box blur for the background avatar trick.
    ...
    for (let x = 0; x < w; x++) {
      ...
      for (let yy = -r; yy <= r; yy += 2) {
        for (let xx = -r; xx <= r; xx += 2) {
          ...
          const i = (ny * w + nx) * 4;
          R += s[i]; G += s[i + 1]; B += s[i + 2]; A += s[i + 3]; c++;
        }
      }
      const j = (y * w + x) * 4;
      d[j] = R / c; d[j + 1] = G / c; d[j + 2] = B / c; d[j + 3] = A / c;
    }
  }
  ctx.putImageData(out, 0, 0);
}

async function generateWelcomeCard({ username, memberCount, joinDate, avatarURL }) {
  const W = 1100, H = 360;
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext("2d");

  // 1) Background: blurred zoomed avatar (per-member uniqueness)
  let avatarBuf = avatarURL ? await fetchBuffer(avatarURL) : null;
  let avatarImg = null;
  if (avatarBuf) {
    try { avatarImg = await loadImage(avatarBuf); } catch { avatarImg = null; }
  }
  if (avatarImg) {
    ctx.drawImage(avatarImg, -100, -200, W + 200, H + 400);
    boxBlur(ctx, W, H, 18);
  } else {
    // Fallback gradient if avatar isn't loadable
    const base = ctx.createLinearGradient(0, 0, W, H);
    base.addColorStop(0, "#1a1d24");
    base.addColorStop(1, "#0a0c11");
    ctx.fillStyle = base;
  ...
  og.addColorStop(0.55, "rgba(0,0,0,0.65)");
  og.addColorStop(1, "rgba(0,0,0,0.75)");
  ctx.fillStyle = og;
  ctx.fillRect(0, 0, W, H);
  ...
  // 4) Grey brand stripe (visible on dark bg, matches the grey embed theme)
  ctx.fillStyle = "#72767d";
  ctx.fillRect(0, 0, 5, H);

  // 5) Avatar with orange glow + white ring
  const AV = 200;
  const AX = 80, AY = (H - AV) / 2;
  const cx = AX + AV / 2, cy = AY + AV / 2;

  const ring = ctx.createRadialGradient(cx, cy, AV / 2, cx, cy, AV / 2 + 22);
  ring.addColorStop(0, "rgba(138,141,147,0.45)");
  ring.addColorStop(1, "rgba(138,141,147,0)");
  ctx.fillStyle = ring;
  ctx.beginPath(); ctx.arc(cx, cy, AV / 2 + 22, 0, Math.PI * 2); ctx.fill();

  ctx.beginPath(); ctx.arc(cx, cy, AV / 2 + 6, 0, Math.PI * 2);
  ctx.fillStyle = "#ffffff"; ctx.fill();

  if (avatarImg) {
    ctx.save();
    ctx.beginPath(); ctx.arc(cx, cy, AV / 2, 0, Math.PI * 2); ctx.closePath(); ctx.clip();
    ctx.drawImage(avatarImg, AX, AY, AV, AV);
    ctx.restore();
  } else {
    ctx.save();
    ctx.beginPath(); ctx.arc(cx, cy, AV / 2, 0, Math.PI * 2); ctx.closePath(); ctx.clip();
    const ag = ctx.createLinearGradient(AX, AY, AX + AV, AY + AV);
    ag.addColorStop(0, "#3a4555"); ag.addColorStop(1, "#1c2230");
    ctx.fillStyle = ag; ctx.fillRect(AX, AY, AV, AV);
    ctx.fillStyle = "#ffffff"; ctx.font = "600 100px sans-serif";
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText(username.charAt(0).toUpperCase(), cx, cy);
    ctx.restore();
  }

  // 6) Text block (right of avatar)
  ...
  let ex = TX;
  for (const ch of "NEW MEMBER") {
    ctx.fillText(ch, ex, AY + 30);
    ex += ctx.measureText(ch).width + 4;
  }
  ctx.beginPath(); ctx.arc(ex + 4, AY + 24, 4, 0, Math.PI * 2); ctx.fill();

  // Headline
  ctx.fillStyle = "#ffffff";
  ctx.font = "700 56px sans-serif";
  ctx.fillText("Welcome to Wezzy", TX, AY + 95);

  // Username (truncate if needed)
  ctx.fillStyle = "#8a8d93";
  ctx.font = "600 30px sans-serif";
  let name = "@" + username;
  while (ctx.measureText(name).width > W - TX - 60 && name.length > 4) {
    name = name.slice(0, -2) + "…";
  }
  ctx.fillText(name, TX, AY + 140);

  // Divider
  ctx.strokeStyle = "rgba(255,255,255,0.12)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(TX, AY + 170); ctx.lineTo(W - 80, AY + 170);
  ctx.stroke();

  // Meta row: MEMBER #N | JOINED date
  const META_Y = AY + 205;
  ctx.fillStyle = "rgba(255,255,255,0.45)";
  ctx.font = "600 13px sans-serif";
  ctx.fillText("MEMBER", TX, META_Y);
  ctx.fillStyle = "#ffffff";
  ctx.font = "600 26px sans-serif";
  ctx.fillText(`#${memberCount.toLocaleString()}`, TX, META_Y + 28);

  const sepX = TX + 180;
  ctx.strokeStyle = "rgba(255,255,255,0.12)";
  ctx.beginPath(); ctx.moveTo(sepX, META_Y - 14); ctx.lineTo(sepX, META_Y + 35); ctx.stroke();

  ctx.fillStyle = "rgba(255,255,255,0.45)";
  ctx.font = "600 13px sans-serif";
  ctx.fillText("JOINED", sepX + 22, META_Y);
  ctx.fillStyle = "#ffffff";
  ctx.font = "600 26px sans-serif";
  ctx.fillText(joinDate, sepX + 22, META_Y + 28);

  // 7) Brand mark bottom-right
  ctx.textAlign = "right";
  ctx.fillStyle = "rgba(255,255,255,0.35)";
  ctx.font = "500 14px sans-serif";
  ctx.fillText("wezzy.store", W - 30, H - 25);
  ctx.beginPath(); ctx.fillStyle = "#8a8d93";
  const brandX = W - 30 - ctx.measureText("wezzy.store").width - 12;
  ctx.arc(brandX, H - 30, 4, 0, Math.PI * 2); ctx.fill();

  return await canvas.encode("png");
}

// ---------------------------------------------------------------------------
// Welcome — image card in WELCOME_CHANNEL_ID when a new member joins
// ---------------------------------------------------------------------------
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    await channel.send({
      content: `🎉 ${member} just joined.`,
      files: [attachment],
      allowedMentions: { users: [member.id] },
    });

    console.log(`[WELCOME] Greeted ${member.user.tag} (member #${member.guild.memberCount})`);
  } catch (err) {
    console.error("[WELCOME] Failed to greet new member:", err.message);
  }
});
